...
| Automation | Vitest (unit + jsdom UI + **coverage gates** on domain/messaging), Playwright (MV3 + navigate→stale + Nano-off curated + **Smart invite badge→accept** + **Settings pause** + **Smart revoke→Manual** + homepage empty-state + **axe a11y** options/panel), GitHub Actions (`test:ci`, Node 20.19+) |
