...
> **WIP — M3 Smart mode in progress; M2 Nano hardware smoke still open.** Manual core (M1) is buildable and covered by Vitest/Playwright/CI. Nano UX and Smart host-permission education + grant/revoke are on `main`. Engagement thresholds + content tracker are landing next (inject after Smart grant). Chrome Web Store packaging (M4) is still ahead.
...
| Engagement | Domain + content tracker (article/product bars, eligibility, once-per-page); Vitest covered; fixture fire / Smart inject still TBD |
...
- Full Smart mode (invites, badge, domain caps) — engagement thresholds exist; host-permission education/grant/revoke is on `main`; inject/caps still WIP
